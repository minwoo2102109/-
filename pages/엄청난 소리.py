...
# 인터넷에서 바로 스트리밍 가능한 MP3 링크
SOUNDS = {
    "🚨 저세상 지구멸망 경보음": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3",
    "📢 8비트 레트로 싸이렌": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3",
    "🐓 분노조절장애 수탉 소리": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3"
}
...
st.subheader("상상치도 못한 소리와 정신 번쩍 문제의 콜라보")
...
if st.button("⏰ 강제 기상 모드 가동", type="primary"):
    st.session_state.alarm_triggered = False
    st.success(f"[{target_time.strftime('%H:%M')}]에 알람이 예약되었습니다. 이 창을 닫지 마세요!")
...
if st.session_state.alarm_triggered:
    ...
    # 가장 안전한 오디오 재생 방식 (삼중 따옴표 제거)
    st.audio(st.session_state.selected_sound, format="audio/mp3", autoplay=True, loop=True)
    
    st.warning("⚠️ 문제를 맞추기 전까지는 이 소리는 절대 멈추지 않습니다!")
    
    # 수학 문제 풀기 구역
    st.markdown("### 🧮 미션: 다음 문제를 해결하고 침대를 탈출하세요!")
    st.subheader(f"👉 {st.session_state.math_problem}")
    
    user_ans = st.number_input("정답 입력:", step=1, value=0)
    
    if st.button("🔓 알람 해제하기"):
        if user_ans == st.session_state.math_answer:
            st.balloons()
            st.success("🎉 정답입니다! 알람이 해제되었습니다.")
            st.session_state.alarm_triggered = False
            st.session_state.math_problem = None
            st.session_state.math_answer = None
            time.sleep(2)
